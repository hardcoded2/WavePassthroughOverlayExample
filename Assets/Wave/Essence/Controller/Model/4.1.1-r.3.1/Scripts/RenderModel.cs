...
namespace Wave.Essence.Controller.Model
{
	public class RenderModel : MonoBehaviour
	{
		...
		bool checkShowModel()
		{
			...
			bool systemFocus = Interop.WVR_IsInputFocusCapturedBySystem();

			bool validPoseState = false;
			InputDevice device = InputDevices.GetDeviceAtXRNode(node);

			if (InputDevices.GetDeviceAtXRNode(node) != null)
			{
				InputDevices.GetDeviceAtXRNode(node).TryGetFeatureValue(new InputFeatureUsage<bool>("IsTracked"), out validPoseState);
			}

			if (checkInteractionMode)
			{
				show = (ClientInterface.InteractionMode == XR_InteractionMode.Controller) && !systemFocus && validPoseState;
			} else
			{
				show = (!systemFocus && validPoseState);
			}
